...
local function eject_dat_chk()
    ...
    repeat
        env.posedge()
        if deq_req:fire() then
            deq_req.bits:expect_hex_str_v2(req_q:front()[2])
            req_q:pop()
            req_cnt = req_cnt + 1
        end
        if deq_rsp:fire() then
            deq_rsp.bits:expect_hex_str_v2(rsp_q:front()[2])
            rsp_q:pop()
            rsp_cnt = rsp_cnt + 1
        end
        if deq_dat:fire() then
            deq_dat.bits:expect_hex_str_v2(dat_q:front()[2])
            dat_q:pop()
            dat_cnt = dat_cnt + 1
        end
        if deq_snp:fire() then
            deq_snp.bits:expect_hex_str_v2(snp_q:front()[2])
            snp_q:pop()
            snp_cnt = snp_cnt + 1
        end
        cnt = req_cnt + rsp_cnt + dat_cnt + snp_cnt
