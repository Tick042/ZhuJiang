...
class EjectBuffer[T <: Flit](gen: T, size: Int)(implicit p: Parameters) extends ZJModule {
  ...
  io.deq <> queue.io.deq
  private val meetEscape = io.enq.valid && !io.enq.ready
  ...
  }.elsewhen(meetEscape && rsvdSel.valid) {
    rsvdNumReg := rsvdNumReg + 1.U
  }
  ...
  for(idx <- rsvdValids.indices) {
    ...
    val doRsv = rsvdSel.valid & rsvdSel.bits(idx) & meetEscape
  ...
  queue.io.enq.bits := io.enq.bits.flit
